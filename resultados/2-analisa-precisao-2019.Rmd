...
```{r}
max_op30 = max(sentimentos %>%  select(sentimento_op30))
min_op30 = min(sentimentos %>%  select(sentimento_op30))
max_sent = max(sentimentos %>%   select(sentimento_sent))
min_sent = min(sentimentos %>%   select(sentimento_sent))


normaliza = function(lexico, polaridade) {
    if(lexico == "sentimento_op30"){
        return (((polaridade - max_op30)/(min_op30-max_op30))*4 + 1 )
    } else {
        return (((polaridade - max_sent)/(min_sent-max_sent))*4 + 1 )
    }
}
...
reclamacoes_polarizada = reclamacoes_polarizada %>% 
    mutate(erro = (grau_insatisfacao - polaridade_normalizada)**2)
```
...
```{r}
sentimentos %>% 
    ggplot(aes(x = conv_op_30, y = conv_sent)) + 
...
reclamacoes_polarizada %>% 
    ggplot(aes(x = grau_insatisfacao, y = polaridade_normalizada, group = grau_insatisfacao)) + 
    geom_abline(slope = 1, intercept = 0, color = "grey") + 
    geom_jitter(alpha = .7)  + 
    facet_wrap(~ lexico)
```
...
```{r}
reclamacoes_polarizada %>% 
    ggplot(aes(x = grau_insatisfacao, y = erro, group = grau_insatisfacao)) + 
    geom_jitter(alpha = .5)  +
    facet_wrap(~ lexico)
```
...
reclamacoes_polarizada = reclamacoes_polarizada %>% mutate(orgao.dummy = if_else(orgao == "INSS", 1, 0))
